...
- [ ] создать .gitignore (включить home/ — там креды!)
- [ ] создать .dockerignore для исключения лишних файлов
- [ ] проверить структуру через ls

### Task 2: Создать Dockerfile — базовый слой
- [ ] создать Dockerfile с multi-stage структурой
- [ ] настроить базовый Ubuntu 24.04 образ
- [ ] установить системные зависимости (curl, git, build-essential, etc.)
- [ ] собрать образ — проверить что этап проходит

### Task 3: Добавить Go в Dockerfile
- [ ] добавить stage для копирования Go из официального образа
- [ ] настроить GOPATH и PATH
- [ ] собрать образ — проверить `go version`

### Task 4: Добавить Node.js в Dockerfile
- [ ] установить Node.js 22 LTS через NodeSource или официальный образ
- [ ] установить pnpm глобально
- [ ] собрать образ — проверить `node --version`, `npm --version`, `pnpm --version`

### Task 5: Добавить Python в Dockerfile
- [ ] установить Python 3.12 из deadsnakes PPA или системного репозитория
- [ ] установить pip, venv
- [ ] установить poetry/uv для управления зависимостями
- [ ] собрать образ — проверить `python3 --version`, `pip --version`

### Task 6: Установить AI-агенты
- [ ] установить claude-code через npm (`@anthropic-ai/claude-code`)
- [ ] установить codex через npm (`@openai/codex`)
- [ ] собрать образ — проверить `claude --version`, `codex --version`

### Task 7: Установить ralphex
- [ ] установить ralphex из GitHub releases или через go install
- [ ] собрать образ — проверить `ralphex --version`

### Task 8: Финализировать образ
- [ ] настроить рабочую директорию (/workspace)
- [ ] настроить entrypoint/cmd
- [ ] добавить non-root пользователя `developer` (UID 1000) с home в /home/developer
- [ ] home будет монтироваться с хоста — не создавать файлы внутри образа
- [ ] собрать финальный образ — полная проверка

### Task 9: Создать Makefile
- [ ] добавить `make build` — сборка образа
- [ ] добавить `make run` — запуск контейнера с маунтами:
  - `./home` → `/home/developer` (персистентный home)
  - текущая директория (или PROJECT=...) → `/workspace`
- [ ] добавить `make shell` — интерактивный шелл в контейнере
- [ ] добавить `make test` — запуск smoke-тестов
- [ ] добавить `make init` — создать ./home если не существует
